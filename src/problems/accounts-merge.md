...
impl Solution {
    pub fn accounts_merge(accounts: Vec<Vec<String>>) -> Vec<Vec<String>> {
        let mut email_to_accounts: HashMap<String, Vec<usize>> = HashMap::new();

        for (i, account) in accounts.iter().enumerate() {
            for email in &account[1..] {
                email_to_accounts.entry(email.clone()).or_default().push(i);
            }
        }
        ...
        fn dfs(
            i: usize,
            accounts: &[Vec<String>],
            email_to_accounts: &HashMap<String, Vec<usize>>,
            ...
            for email in &accounts[i][1..] {
                if emails.insert(email.clone()) {
                    if let Some(neighbors) = email_to_accounts.get(email) {
                        for &j in neighbors {
                            dfs(j, accounts, email_to_accounts, visited, emails);
                        }
                    }
                }
            }

---
id: "minimum-swaps-to-avoid-forbidden-values"
...
var minSwaps = function(nums, forbidden) {
    const nmap = new Map(), fmap = new Map(), dismap = new Map();
    let dis = 0;

    for (let i = 0; i < nums.length; i++) {
        nmap.set(nums[i], (nmap.get(nums[i]) || 0) + 1);
        fmap.set(forbidden[i], (fmap.get(forbidden[i]) || 0) + 1);

        if (nums[i] === forbidden[i]) {
            dis++;
            dismap.set(nums[i], (dismap.get(nums[i]) || 0) + 1);
        }
    }

    for (const [key, value] of nmap) {
        if (value + (fmap.get(key) || 0) > nums.length) return -1;
    }

    if (!dis) return 0;

    let max = 0;
    for (const value of dismap.values()) {
        max = Math.max(max, value);
    }

    return Math.max(Math.floor((dis + 1)/2), max)
};
```

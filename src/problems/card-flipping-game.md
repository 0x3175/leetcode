...
func flipgame(fronts []int, backs []int) int {
	impossibles := make(map[int]bool)
	min := 3000 // a number larger than max value in fronts and backs
	for i := 0; i < len(fronts); i++ {
		if fronts[i] == backs[i] {
			impossibles[fronts[i]] = true
		}
	}
	for i := 0; i < len(fronts); i++ {
		if fronts[i] < min && !contains(impossibles, fronts[i]) {
			min = fronts[i]
		}
		if backs[i] < min && !contains(impossibles, backs[i]) {
			min = backs[i]
		}
	}
	return min % 3000
...
func contains(d map[int]bool, v int) bool {
	_, ok := d[v]
	return ok
}
```
